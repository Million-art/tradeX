...
async def handle_welcome_message(message: types.Message):
    ...
    # Ask the user to provide the media (photo, video, or GIF)
    await bot.reply_to(message, "Please upload a photo, video, or GIF for the welcome message.")
    USER_SESSIONS[user_id] = {"state": "awaiting_media", "welcome_message": welcome_message}

# Handle the user's media upload
@bot.message_handler(content_types=['photo', 'video', 'animation', 'document'])
async def handle_media(message: types.Message):
    user_id = message.from_user.id

    # Check if the user is in the "awaiting_media" state
    user_session = USER_SESSIONS.get(user_id, {})
    if user_session.get("state") == "awaiting_media":
